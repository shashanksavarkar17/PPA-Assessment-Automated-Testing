...
class SummaryPage(BasePage):
    def wait_for_page_load(self):
        # We wait until the first "Solve" button is visible to confirm we're back on the dashboard.
        self.helpers.wait_for_element((By.XPATH, "//button[contains(text(), 'Solve')]"))
        
    def start_section(self, section_idx):
        try:
            # Let's locate all the section solve buttons and click the target one via JS.
        ...
        try:
            ...
            for r in rows:
                ...
                if th and "Section:" in th[0].text:
                    # Found a new section header row. Let's start grouping questions under it.
                    curr_sec = th[0].text[th[0].text.find("Section:"):].strip()
                    data[curr_sec] = []
                elif curr_sec:
                    # This must be a question detail row. Parse standard columns: ID, Description, Status.
