    ...
    try:
        ...
        for sec_idx, (sec_name, q_count) in enumerate(section_data.items(), 1):
            ...
            for _ in range(q_count):
                ...
                time.sleep(0.3)
                q_type, q_text = pages['question'].get_question_type(), pages['question'].get_question_text()
                
                # Direct route to solve MCQ or Coding question.
                if q_type == 'MCQ':
...
def _solve_coding(page, solver, report, sec_name, q_text, q_idx):
    # Retrieve Gemini C++ program, inject, run/submit, and proceed.
